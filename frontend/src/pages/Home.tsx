...
// Type definitions for the UI components data structure
interface PageData {
  id: string;
  name: string;
  description: string;
...
const mainPageId = "7gx6FKvT6NwQwvkM";
...
const Home: React.FC = () => {
  ...
  const typedData = componentsData as ComponentsData;
  const pages = typedData?.pages ?? [];
  console.log("pages array:", pages);

  // Find page by route path or fallback to main page
  let page: PageData | undefined = pages.find((p) => p.route_path === location.pathname);
  
  // If not found by route, use main page or first page
  if (!page) {
    page = pages.find((p) => (mainPageId ? p.id === mainPageId : false)) ?? pages[0];
  }
